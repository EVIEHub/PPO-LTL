...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals['dones'][0]:
            ...
            # Violation breakdown
            if 'violation/collision' in self.locals['infos'][0]:
                self.logger.record("violation/collision", self.locals['infos'][0]['violation/collision'])
                self.logger.record("violation/off_track", self.locals['infos'][0]['violation/off_track'])
                self.logger.record("violation/lane_invasion", self.locals['infos'][0]['violation/lane_invasion'])
                self.logger.record("violation/heading", self.locals['infos'][0]['violation/heading'])
                self.logger.record("violation/weaving", self.locals['infos'][0]['violation/weaving'])
                self.logger.record("violation/overspeed", self.locals['infos'][0]['violation/overspeed'])
                self.logger.record("violation/steer_jerk", self.locals['infos'][0]['violation/steer_jerk'])
...
def parse_wrapper_class(wrapper_class_str: str):
    ...
    """
    wrap_class, rest = wrapper_class_str.split("_", 1)
    tokens = [t for t in (rest.split(";") if ";" in rest else rest.split("_")) if t]
    # support key=value pairs + numeric/boolean coercion
    def _coerce(v: str):
        lv = v.lower()
        if lv == "true":
            return True
    ...
    for t in tokens:
        if "=" in t:
            k, val = t.split("=", 1)
            kv_params[k] = _coerce(val)
        else:
            pos_params.append(_coerce(t))

    if wrap_class == "HistoryWrapperObsDict":
        return HistoryWrapperObsDict, pos_params
    elif wrap_class == "FrameSkip":
        return FrameSkip, pos_params
    elif wrap_class == "ShieldWrapper":
        from shielding.shield_wrapper import ShieldWrapper
        def _wrap(env, *args):
            return ShieldWrapper(env, **kv_params)
        return _wrap, []
    elif wrap_class == "TableShieldWrapper":
        from shielding.table_shield_wrapper import TableShield, CarlaShieldWrapper
        shield = TableShield.load_pkl("shield_table.pkl")
        def _wrap_table(env, *args):
            return CarlaShieldWrapper(env, shield)
        return _wrap_table, []
