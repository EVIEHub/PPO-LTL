...
class CarlaRouteEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000,
        ...
        if start_carla:
            # Get CARLA root from environment variable, default to home directory
            CARLA_ROOT = os.environ.get('CARLA_ROOT', os.path.expanduser('~'))
        ...
        self.discrete_actions = discrete_actions  # expose for ShieldWrapper lookup
    ...
    def reset(self, is_training=False):
        ...
        #CRL code
        self._lane_invasion_steps = 0  # 侵线惩罚的持续步数（秒转步）
        self._overspeed_steps = 0  # 超速持续步数
        self._low_speed_steps = 0  # 低速持续步数
        self._last_steer = 0.0  # 上一时刻方向盘
        self._last_throttle = 0.0  # 上一时刻油门
        # === 新增：碰撞持续窗口 + 计数器兜底 ===
        self._collision_steps = 0  # 碰撞后维持多少步算“在碰撞”
        self._prev_collision_num = 0  # 兜底：用累计计数器的差分识别新碰撞
        # === Cost debug counter ===
        self._cost_hits = {
            k: 0 for k in [
                "collision", "off_track", "lane_invasion",
                "heading", "weaving", "overspeed",
                "low_speed", "steer_jerk"
            ]
        }
        self._cost_total_steps = 0
    ...
    def step(self, action):
        ...
        ##CRL code
        # === 新增：计算约束成本 cost ===
        # === 约束与成本（只影响 info，不改 reward） ===
        speed = float(self.vehicle.get_speed())
        steer_now = float(self.vehicle.control.steer)
        throttle_now = float(self.vehicle.control.throttle)

        # 现有阈值都来自 config.reward_params，和reward 体系保持一致
        max_distance = CONFIG.reward_params.max_distance
        max_std_center_lane = CONFIG.reward_params.max_std_center_lane
        max_angle_center_lane = CONFIG.reward_params.max_angle_center_lane  # 度
        max_speed = CONFIG.reward_params.max_speed

        # 1) 碰撞（最重）
        if self.collision_num > self._prev_collision_num:
            self._collision_steps = int(1.0 * self.fps)
        self._prev_collision_num = self.collision_num

        collision_now = (self._collision_steps > 0) or bool(self.collision_state)
        if self._collision_steps > 0:
            self._collision_steps -= 1
        # 2) 严重偏离中心线（越界）
        off_track = abs(self.distance_from_center) > max_distance
        # 3) 侵线（来自 _on_invasion；有衰减窗口）
        lane_invasion_active = self._lane_invasion_steps > 0
        if lane_invasion_active:
            self._lane_invasion_steps -= 1
        # 4) 朝向错误/逆向：与当前车道中心线夹角超阈值
        heading_violation = abs(angle) > np.deg2rad(max_angle_center_lane)
        # 5) 蛇形/抖动：中心偏离的滑动标准差过大
        std_dev = np.std(self.distance_from_center_history)
        weaving_violation = std_dev > max_std_center_lane
        # 6) 超速持续 >=1 秒
        if speed > max_speed:
            self._overspeed_steps += 1
        else:
            self._overspeed_steps = 0
        overspeed_persist = self._overspeed_steps >= int(1.0 * self.fps)

        # 8) 急打方向（速度>5km/h 时连续帧转角突变）
        steer_jerk = (abs(steer_now - self._last_steer) > 0.5) and (speed > 5.0)

        # 记录当前以供下一步比较
        self._last_steer = steer_now
        self._last_throttle = throttle_now

        # === 成本聚合（可调权重，支持通过 CONFIG.cost_weights 覆盖）===
        ...
        # 读取可选的 config 权重覆盖
        try:
            cfg_weights = dict(CONFIG.cost_weights) if hasattr(CONFIG, 'cost_weights') and CONFIG.cost_weights is not None else {}
        except Exception:
            cfg_weights = {}
        cost_weights = {k: cfg_weights.get(k, v) for k, v in default_cost_weights.items()}

        cost_components = {
            "collision": (cost_weights["collision"] if collision_now else 0.0),
            "off_track": (cost_weights["off_track"] if off_track else 0.0),
            "lane_invasion": (cost_weights["lane_invasion"] if lane_invasion_active else 0.0),
            "heading": (cost_weights["heading"] if heading_violation else 0.0),
            "weaving": (cost_weights["weaving"] if weaving_violation else 0.0),
            "overspeed": (cost_weights["overspeed"] if overspeed_persist else 0.0),
            "steer_jerk": (cost_weights["steer_jerk"] if steer_jerk else 0.0),
        }
        ...
        info = {
            ...
            "cost": cost,
            # === Violation breakdown ===
            "violation/collision": cost_components["collision"],
            "violation/off_track": cost_components["off_track"],
            "violation/lane_invasion": cost_components["lane_invasion"],
            "violation/heading": cost_components["heading"],
            "violation/weaving": cost_components["weaving"],
            "violation/overspeed": cost_components["overspeed"],
            "violation/steer_jerk": cost_components["steer_jerk"],
        }

        # === Debug: count cost hits ===
        for k, v in cost_components.items():
            if v > 0:
                self._cost_hits[k] += 1
        self._cost_total_steps += 1

        # 每 200 步打印一次命中率
        if self._cost_total_steps % 200 == 0:
            rates = {
                k: round(self._cost_hits[k] / self._cost_total_steps, 3)
                for k in self._cost_hits
            }
            print(f"[COST DEBUG] hit_rates over {self._cost_total_steps} steps: {rates}", flush=True)
    ...
    def _on_collision(self, event):
        if get_actor_display_name(event.other_actor) != "Road":
            ...
            self._collision_steps = int(1.0 * self.fps)  # <<< 新增：碰撞后维持1秒“为真”
    ...
    def _on_invasion(self, event):
        ...
        # 新增：触发 2 秒（按 fps 转为步数）的侵线惩罚窗口
        self._lane_invasion_steps = int(2.0 * self.fps)
